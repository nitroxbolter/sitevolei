...
$sql_grupos = "SELECT id, nome FROM torneio_grupos WHERE torneio_id = ? AND (nome = '2ª Fase - Ouro A' OR nome = '2ª Fase - Ouro B')";
$stmt_grupos = executeQuery($pdo, $sql_grupos, [$torneio_id]);
$grupos = $stmt_grupos ? $stmt_grupos->fetchAll(PDO::FETCH_ASSOC) : [];

if (empty($grupos)) {
    echo json_encode(['success' => false, 'message' => 'Grupos Ouro A e Ouro B não encontrados.']);
    exit();
}
...
foreach ($grupos as $grupo) {
    $grupo_nomes[$grupo['id']] = str_replace('2ª Fase - ', '', $grupo['nome']);
}
...
// Buscar classificação de Ouro A e Ouro B combinados
// Usar dados dos jogos todos contra todos finalizados
$times_classificacao = [];

foreach ($grupos as $grupo) {
    $grupo_id = (int)$grupo['id'];
    $grupo_nome = $grupo_nomes[$grupo_id];
