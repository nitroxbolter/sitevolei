...
// Remover integrante
$sql = "DELETE FROM torneio_time_integrantes WHERE time_id = ? AND participante_id = ?";
$result = executeQuery($pdo, $sql, [$time_id, $participante_id]);

if ($result) {
    echo json_encode(['success' => true, 'message' => 'Integrante removido do time!']);
} else {
    echo json_encode(['success' => false, 'message' => 'Erro ao remover integrante.']);
}
?>
