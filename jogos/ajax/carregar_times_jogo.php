...
if (!$sou_admin && !isAdmin($pdo, $_SESSION['user_id'])) {
    echo json_encode(['success' => false, 'message' => 'Apenas o administrador pode ver times.']);
    exit();
}

// Buscar times do jogo
$sql = "SELECT * FROM sistema_pontuacao_times WHERE jogo_id = ? ORDER BY ordem, nome";
$stmt = executeQuery($pdo, $sql, [$jogo_id]);
...
foreach ($times as &$time) {
    $sql = "SELECT u.id, u.nome, u.foto_perfil
            FROM sistema_pontuacao_time_jogadores stj
            JOIN usuarios u ON u.id = stj.usuario_id
            WHERE stj.time_id = ?
            ORDER BY u.nome";
    $stmt = executeQuery($pdo, $sql, [$time['id']]);
    $time['jogadores'] = $stmt ? $stmt->fetchAll() : [];
}
