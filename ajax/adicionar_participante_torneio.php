...
try {
    if ($tipoTorneio === 'grupo' && !empty($participantes)) {
        ...
        if ($tem_ordem) {
            ...
            executeQuery($pdo, $sql, [$torneio_id, $nome_avulso, $totalAtual + 1]);
        } else {
            $sql = "INSERT INTO torneio_participantes (torneio_id, nome_avulso) VALUES (?, ?)";
            executeQuery($pdo, $sql, [$torneio_id, $nome_avulso]);
        }
    } else {
    ...
    $pdo->commit();
    echo json_encode(['success' => true, 'message' => 'Participante(s) adicionado(s) com sucesso!']);
} catch (Exception $e) {
    $pdo->rollBack();
