...
if ($status !== '') {
    ...
    // Se não especificar status, mostrar apenas ativos (incluindo 'Criado')
    $sql .= " AND t.status IN ('Criado', 'Inscrições Abertas', 'Em Andamento')";
}

$sql .= " GROUP BY t.id ORDER BY t.data_inicio ASC";
...
if (empty($torneios)) {
    ...
    foreach ($torneios as $torneio) {
        ...
        $html .= '<div class="card h-100 torneio-card">';
        ...
        $html .= '<strong>Grupo:</strong> ' . htmlspecialchars($torneio['grupo_nome'] ?: 'Avulso');
        $html .= '</p>';
